...
def convert_str(text):
    try:
        ret1 = text.decode("gbk").encode("utf-8")
    except:
        ret1 = None

    try:
        ret2 = text.decode("shift-jis").encode("utf-8")
    except:
        ret2 = None

    try:
        ret3 = text.decode("big5").encode("utf-8")
    except:
        ret3 = None
...
def check_unsupported(filename):
    ...
    try:
        ...
        rtassert(False, "can't open tja file.")
    END_cnt = 0
    for line in file:
...
def get_meta_data(filename):
    ...
    for line in file:
        ...
        if vname == "TITLE":
            ...
            OFFSET = float(vval)
        elif vname == "DEMOSTART":
            DEMOSTART = float(vval)
        elif vname == "COURSE":
            COURSE = vval

def add_default_timing_point():
    global TimingPoints
    tm = {"offset": -OFFSET * 1000.0, "redline": True, "scroll": 1.0, "measure": 4.0, "GGT": False, "bpm": BPM}
...
def create_new_tm():
    ...
    tm = {"offset": int(curr_time), "redline": None, "scroll": last_green_tm and last_green_tm["scroll"] or 1.0,
          "measure": last_tm["measure"], "GGT": last_tm["GGT"], "bpm": last_red_tm["bpm"]}
...
def handle_a_bar():
    ...
    if False and debug_mode:
        ...
        p1 = "%6d %2.1f %2d %s" % (int(curr_time), get_last_tm()["measure"], len(pure_data), "".join(pure_data))

        p2 = "%.4f %.2f" % (get_last_tm()["bpm"], get_t_unit(get_last_tm(), tot_note) * tot_note)
        print(p1)
    ...
    for data in bar_data:
        if isinstance(data, string_types):  # note
            note_cnt += 1
            if data == "0" or \
                    (lasting_note is not None and data != '8'):
                curr_time += get_t_unit(get_last_tm(), tot_note)
                continue
            add_a_note(data, curr_time)
            if print_each_note:
                print(note_cnt, data, curr_time, bak_curr_time + note_cnt * get_t_unit(get_last_tm(), tot_note),
                      get_t_unit(get_last_tm(), tot_note))
            curr_time += get_t_unit(get_last_tm(), tot_note)
        else:  # cmd
            real_do_cmd(data)
    bar_data = []
...
def write_TimingPoints():
    print("[TimingPoints]")
    for tm in TimingPoints:
        timing = 60000.0 / tm["bpm"] if tm["redline"] else -100 / tm["scroll"]
        print(
            "%d,%f,%d,1,0,100,%d,%d" % (int(tm["offset"]), timing, int(round(tm["measure"])), tm["redline"], tm["GGT"]))
...
def write_HitObjects():
    ...
    for ho in HitObjects:
        ...
        if ho[0] == CIRCLE:
            ...
            print("%d,%d,%d,%d,%d,L|%d:%d,%d,%f" %
                  (
                      CircleX, CircleY, int(get_real_offset(ln[2])), ln[0], ln[1],
            ...
            print("%d,%d,%d,%d,%d,%d" %
                  (
                      CircleX, CircleY, int(get_real_offset(ln[2])), ln[0], ln[1], int(get_real_offset(ho[2]))
                  ))
            lasting_note = None
    print()
